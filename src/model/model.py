...
class OCRModel(object):
    ...
    def train(self, optimizer, epoch, dataloader):
        ...
        with tqdm(dataloader) as pbar:
            for batch_index, (data, target, input_lengths, target_lengths) in enumerate(pbar):
                ...
                loss = self.criterion(output_log_softmax,
    ...
    def evolution(self, epochs=10, lr=1e-4, optimizer=None):
        train_set = OCRDataset(True, self.normalizer,
                               self.input_shape[2] // (2**4))
        valid_set = OCRDataset(False, self.normalizer,
                               self.input_shape[2] // (2**4))

        train_loader = DataLoader(
            train_set, batch_size=self.batch_size, num_workers=2, collate_fn=self.collate_fn)
        valid_loader = DataLoader(
            valid_set, batch_size=self.batch_size, num_workers=2, collate_fn=self.collate_fn)
        ...
        for epoch in range(1, epochs + 1):
            ...
            self.writer.add_scalars(
                'epoch/loss', {'train': train_loss, 'valid': valid_loss}, epoch)
            self.writer.add_scalars(
                'epoch/acc', {'train': train_acc, 'valid': valid_acc}, epoch)
            self.writer.add_scalars(
                'epoch/error_rate', {'train': 1 - train_acc, 'valid': 1 - valid_acc}, epoch)
